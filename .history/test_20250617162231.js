...
async function runAllTests() {
  ...
  try {
    console.log('\n🔹 Categories');
try {
  const data = await gm.categories.getCategoriesController();
  console.log(data);
} catch (err) {
  console.error('❌ Failed Categories:', err.message || err);
}

console.log('\n🔹 Offer Products');
try {
  const data = await gm.offerProducts.getOfferProductsController(12);
  console.log(data);
} catch (err) {
  console.error('❌ Failed Offer Products:', err.message || err);
}


    console.log('\n🔹 Master Categories');
try {
  const data = await gm.masterCategories.getMasterCategoriesController(12);
  console.log(data);
} catch (err) {
  console.error('❌ Failed Master Categories:', err.message || err);
}
