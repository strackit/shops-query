...
async function runAllTests() {
  ...
  const userId = 101; // make sure this exists in your database
  try {
    const data = await fetchWishlistController(userId);
...
testWishlist();
  ...
  try {
    const result = await gm.wishlist.addToWishlistControlleroWishlist(101, 25); // Make sure both IDs exist
...
const testWishlistRemove = async () => {
  try {
    const result = await removeFromWishlist({
      ...
      productId: 101,  // ✅ product must be in wishlist
      shopId: 25       // ✅ shop must match the one in the wishlist
    });
