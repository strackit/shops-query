...
async function runAllTests() {
  ...
  try {
    ...
    console.log('\n🔹 Cart - Remove');
    console.log(await gm.cart.removeFromCart({ productId: 7660, shopId: 348, userId: null, update: false, quantity: 1 }));

    console.log('\n🔹 Wishlist - Fetch');
    const userId = null; // Replace this with a valid user ID

const testWishlist = async () => {
  try {
    const wishlist = await gm.wishlist.getWishlistController(userId);
  ...
  try {
    const result = await gm.wishlist.addToWishlistController(null,7660 ); // Make sure both IDs exist
...
testWishlistAdd();
    ...
    console.log('\n🔹 Wishlist - Add');
await gm.wishlist.addToWishlistController({ userId: 1, productId: 3128, shopId: 12 });

console.log('\n🔹 Wishlist - Remove');
console.log(await gm.wishlist.removeFromWishlistController({ userId: 1, productId: 3128, shopId: 12 }));
    ...
    console.log('\n🔹 Products by Filters');
    console.log(await gm.productByFilters.getProductsByFiltersController({ shopId: 348, userId: null }));
