// test.js

import testAddToCart from './test_/cart-add.test.js';
import testFetchCart from './test_/cart-fetch.test.js';
import testRemoveFromCart from './test_/cart-remove.test.js';
import fetchCategoriesTest from './test_/categories.test.js';
import testMasterCategory from './test_/masterCategory.test.js';
import testNestedCategory from '.test_/nestedCategory.test.js';
import testOfferProducts from './test_/offerProducts.test.js';
import testOrderUpdates from './test_/orderUpdates.test.js';
import testProductByPrice from './test_/productByPrice.test.js';
import testProductsByCategory from './test_/productsByCategory.test.js';
import testProductReviews from './test_/productReviews.test.js';
import testProducts from './test_/products.test.js';
import testSearchProducts from './test_/searchProducts.test.js';
import testTopProducts from './test_/topProducts.test.js';
import wishlistAddRemoveTest from './test_/wishlistAddRemove.test.js';
import wishlistFetchTest from './test_/wishlistFetch.test.js';
// ... import other test files here ...

async function runAllTests() {
  await testAddToCart();
  await testFetchCart();
  await testRemoveFromCart();
  await fetchCategoriesTest();
  await testMasterCategory();
  await testNestedCategory();
  await testOfferProducts();
  await testOrderUpdates();
  await testProductByPrice();
  await testProductsByCategory();
  await testProductReviews();
  await testProducts();
  await testSearchProducts();
  await testTopProducts();
  await wishlistAddRemoveTest();
  await wishlistFetchTest();

  // ... call all other test functions ...
}

runAllTests()
  .then(() => console.log('\n✅ All tests completed'))
  .catch(err => {
    console.error('\n❌ Test suite failed');
    console.error(err);
    process.exit(1);
  });
