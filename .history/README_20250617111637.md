# 🛍️ GraphQL E-commerce Backend

This project is a modular **GraphQL-based backend** for managing an e-commerce platform. It supports querying and mutating data for categories, products, cart, wishlist, reviews, offers, and real-time order updates.

Built for clean controller-query architecture and easily testable GraphQL module logic.

---
📁 Project Structure

graphql-project/
│
├── src/
│ ├── modules/
│ │ ├── cart/
│ │ │ ├── controller/
│ │ │ ├── mutations/
│ │ │ └── queries/
│ │ ├── categories/
│ │ │ ├── controller/
│ │ │ └── queries/
│ │ ├── masterCategories/
│ │ │ ├── controller/
│ │ │ └── queries/
│ │ ├── nestedCategory/
│ │ │ ├── controller/
│ │ │ └── queries/
 graphql-project/src offerProducts/
 graphql-project/ productReviews/
 graphql-project/ products/
 graphql-project/ wishlist/
 graphql-project/ topProducts/
 graphql-project/productByCategory/
 graphql-project/ productByFilters/
 graphql-project/ productByPrice/
 graphql-project/ searchproducts/
 graphql-project/ orderUpdates/
 graphql-project/ utils/ client.js
 graphql-project/.env
  graphql-project/index.js
  graphql-project/test.js 
   graphql-project/README.md
    graphql-project/package.json 