...
async function runAllTests() {
  ...
  try {
    ...
    const testShopId = 1; // Or use any valid shopId you know exists in your DB

console.log('\n🔹 Get Products');
const productList = await gm.products.getProductsController(testShopId);
console.log(productList);
const productToAdd = productList?.[0];

if (!productToAdd || !productToAdd.id) {
  throw new Error('❌ No valid product found to test Cart Add');
}

console.log('\n🔹 Cart - Add');
console.log(await gm.cart.addToCart({
  productId: productToAdd.id,
  shopId: productToAdd.shopId || testShopId,
  userId: 1,
  quantity: 2,
  update: false,
}));
