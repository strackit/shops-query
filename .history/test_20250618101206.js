...
async function runAllTests() {
  ...
  try {
    ...
    const productToAdd = productList.find(p => p?.id && p?.shopId === shopId);

if (!productToAdd) {
  console.error('❌ Test Error: ❌ No valid product found to test Cart Add');
  return;
}

    console.log('\n🔹 Cart - Add');
    console.log(await gm.cart.addToCart({ productId: 1, shopId: 12, userId: 12, quantity: 2 }));
